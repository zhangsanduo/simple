...
void md5(const uint8_t *initial_msg, size_t initial_len, uint8_t *digest)//MD5加密，参数依次为明文，明文长度，加密结果
 {
    ...
    // 初始化变量
    h0 = 0x67452301;//A
    printf("md5_1_1_2_0:%x\n",h0);
    h1 = 0xefcdab89;//B
    printf("md5_1_1_3_0:%x\n",h1);
    h2 = 0x98badcfe;//C
    printf("md5_1_1_4_0:%x\n",h2);
    h3 = 0x10325476;//D
    printf("md5_1_1_5_0:%x\n",h3);
   ...
   //预处理：
    ...
    //为这个块初始化散列值：
        ...
        for(i = 0; i<64; i++)
        {
            if(i==0)
            {
               printf("md5_2_22_24_%d:l-%d\n",i+1,i+1);
               printf("md5_2_22_1_%d:%x\n",i+1,a);
               printf("md5_2_22_2_%d:%x\n",i+1,b);
               printf("md5_2_22_3_%d:%x\n",i+1,c);
               printf("md5_2_22_4_%d:%x\n",i+1,d);
            }
            else
            {
               printf("md5_2_23_24_%d:l-%d\n",i+1,i+1);
               printf("md5_2_23_1_%d:%x\n",i+1,a);
               printf("md5_2_23_2_%d:%x\n",i+1,b);
               printf("md5_2_23_3_%d:%x\n",i+1,c);
               printf("md5_2_23_4_%d:%x\n",i+1,d);
            }
            if (i < 16)
            {
                f = (b & c) | ((~b) & d);//基本逻辑函数F
            ...
            printf("md5_2_7_5_%d:%x\n",i+1,f);
            printf("md5_2_6_8_%d:%x\n",i+1,a+f);
            printf("md5_2_6_9_%d:%x\n",i+1,w[g]);
            printf("md5_2_10_11_%d:%x\n",i+1,a+f+w[g]);
            printf("md5_2_10_12_%d:%x\n",i+1,k[i]);
            printf("md5_2_13_14_%d:%x\n",i+1,a+f+k[i]+w[g]);
            printf("md5_2_15_16_%d:%x\n",i+1,LEFTROTATE ((a + f + k[i] + w[g]), r[i]));
            temp = d;
            d = c;
            c = b;
            b = b + LEFTROTATE ((a + f + k[i] + w[g]), r[i]);
            a = temp;
            printf("md5_2_17_18_%d:%x\n",i+1,a);
            printf("md5_2_17_19_%d:%x\n",i+1,b);
            printf("md5_2_17_20_%d:%x\n",i+1,c);
            printf("md5_2_17_21_%d:%x\n",i+1,d);
        }
...
int main(int argc, char **argv)
{
    ...
    printf("md5_1_6_10_0:%2.2x%2.2x%2.2x%2.2x\n",result[0],result[1],result[2],result[3]);
    printf("md5_1_7_11_0:%2.2x%2.2x%2.2x%2.2x\n",result[4],result[5],result[6],result[7]);
    printf("md5_1_8_12_0:%2.2x%2.2x%2.2x%2.2x\n",result[8],result[9],result[10],result[11]);
    printf("md5_1_9_13_0:%2.2x%2.2x%2.2x%2.2x\n",result[12],result[13],result[14],result[15]);
    puts("");
    return 0;
}
