...
unsigned char *elan_d = (unsigned char *)&ELAN;
big x,y;
SM3_STATE md;
x=mirvar(0);
y=mirvar(0);
...
memcpy(IDlen,elan_d+1,1);
memcpy(IDlen+1,elan_d,1);
...
{
	return SM2_KeyGeneration_enc(ra,RA);
}
...
Output: K // secret key that A and B wanna shareSA // (option) calculates a hash value SA that initiator B might
verifies
Return: 0: sucess
...
big_to_bytes(SM2_NUMWORD,y1,x1y1+SM2_NUMWORD,TRUE);//--------A4: x1_=2^w+x2 & (2^w-1)--------
expb2 (w,x1_); // x1_=2^w
divide(x1,x1_,tmp); //x1=x1 mod x1_ =x1 & (2^w-1)
add(x1_,x1,x1_);
...
//------------A8:KA=KDF(UX,UY,ZA,ZB,KLEN)----------memcpy(Z+SM2_NUMWORD*2,ZA,SM3_len/8);
memcpy(Z+SM2_NUMWORD*2+SM3_len/8,ZB,SM3_len/8);
SM3_KDF(Z,SM2_NUMWORD*2+SM3_len/4,klen/8,K);
...
Called By:
Input: V // calculated in SM2_KeyEx_Re_IRA // temporary public key received from initiator A
RB // temporary public key received from initiator B
ZA // Z=hash(ELAN_A||ID of A||a ||b||Gx||Gy||PAx||PAy)
ZB // Z=hash(ELAN_B||ID of B||a ||b||Gx||Gy||PBx||PBy)
...
SM3_init (&md);
SM3_process(&md,xV,SM2_NUMWORD);SM3_process(&md,ZA,SM3_len/8);
SM3_process(&md,ZB,SM3_len/8);
SM3_process(&md,x1y1,SM2_NUMWORD*2);
SM3_process(&md,x2y2,SM2_NUMWORD*2);
SM3_done(&md, hash);
...
,0x34,0x28,0xCF,0x60,0x8A,0x5D,0xB8,0xFE,0x5C,0xE0,0x7F,0x15,0x02,0x69,0x40,0xBA,0xE4,0x0E,0x37,0x66,0x29,0xC7,0xAB,0x21,0xE7,0xDB,0x26,0x09,0x22,0x49,0x9D,0xDB,0x11,0x8F,0x07,0xCE,0x8E,0xAA,0xE3,0xE7,0x72,0x0A,0xFE,0xF6,0xA5,0xCC,0x06,0x20,0x70,0xC0};
unsigned char
