#include "zuc.h"
#if defined(_M_IX86) || defined(_M_X64)
#   include <immintrin.h>
#endif
#include <string.h>
#include <stdlib.h>
// Constants.

// S盒
static const uint8_t S0[256] = {
    
};

static const uint8_t S1[256] = {
    
};

// D不变.
static const uint16_t D[16] = {
    0x44D7, 0x26BC, 0x626B, 0x135E, 0x5789, 0x35E2, 0x7135, 0x09AF,
    0x4D78, 0x2F13, 0x6BC4, 0x1AF1, 0x5E26, 0x3C4D, 0x789A, 0x47AC
};

// 纯粹的功能

// 添加31位无符号整数。
static inline uint32_t addition_uint31(const uint32_t a, const uint32_t b)
{
    const uint32_t c = a + b;
    return (c & 0x7FFFFFFF) + (c >> 31);
}

// 循环左移31位无符号整数
static inline uint32_t rotl_uint31(const uint32_t a, const uint32_t shift)
{
    return ((a << shift) | (a >> (31 - shift))) & 0x7FFFFFFF;
}

// 左移32位无符号整数.
#if defined(_M_IX86) || defined(_M_X64)
#    define rotl_uint32 _rotl // Use Intel intrinsics when available.
#else
static inline uint32_t rotl_uint32(const uint32_t a, const uint32_t shift)
{
    return (a << shift) | (a >> (32 - shift));
}
#endif

//线性变换L1。
static inline uint32_t l1(const uint32_t x)
{
    return (x ^ rotl_uint32(x, 2) ^ rotl_uint32(x, 10) ^ rotl_uint32(x, 18) ^ rotl_uint32(x, 24));
}

// 线性变换L2。
static inline uint32_t l2(const uint32_t x)
{
    return (x ^ rotl_uint32(x, 8) ^ rotl_uint32(x, 14) ^ rotl_uint32(x, 22) ^ rotl_uint32(x, 30));
}

// 连接成32位无符号整数.
static inline uint32_t make_uint32(const uint8_t a, const uint8_t b, const uint8_t c, const uint8_t d)
{
    return (((uint32_t)a << 24) | ((uint32_t)b << 16) | ((uint32_t)c << 8) | (uint32_t)d);
}

// 连接成31位无符号整数.
static inline uint32_t make_uint31(const uint8_t a, const uint16_t b, const uint8_t c)
{
    return ((uint32_t)a << 23) | ((uint32_t)b << 8) | (uint32_t)c;
}

// 计算LFSR的下一个值。
static uint32_t lfsr_next(const uint32_t lfsr[16]) {
    uint32_t f = lfsr[0], v = rotl_uint31(lfsr[0], 8);
    f = addition_uint31(f, v);
    v = rotl_uint31(lfsr[4], 20);
    f = addition_uint31(f, v);
    v = rotl_uint31(lfsr[10], 21);
    f = addition_uint31(f, v);
    v = rotl_uint31(lfsr[13], 17);
    f = addition_uint31(f, v);
    v = rotl_uint31(lfsr[15], 15);
    f = addition_uint31(f, v);
    return f;
}

// 不纯粹的功能

// 附加到LSFR.
static inline void lfsr_append(uint32_t lfsr[16], const uint32_t f) {
    memmove(&lfsr[0], &lfsr[1], 15 * sizeof(uint32_t));
    lfsr[15] = f;
}

// 使用初始化模式切换LFSR.
static inline void lfsr_init(uint32_t lfsr[16], const uint32_t u)
{
    lfsr_append(lfsr, addition_uint31(lfsr_next(lfsr), u));
}

// 使用工作模式切换LFSR.
static inline void lfsr_shift(uint32_t lfsr[16]) {
    lfsr_append(lfsr, lfsr_next(lfsr));
}

// 非线性函数F..
static uint32_t f(pzuc_context context,int i,int test)
{
    const uint32_t W = (context->x[0] ^ context->r[0]) + context->r[1];
    const uint32_t W1 = context->r[0] + context->x[1];
    const uint32_t W2 = context->r[1] ^ context->x[2];
    const uint32_t u = l1((W1 << 16) | (W2 >> 16));
    const uint32_t v = l2((W2 << 16) | (W1 >> 16));
    context->r[0] = make_uint32(S0[u >> 24], S1[(u >> 16) & 0xFF],
                     S0[(u >> 8) & 0xFF], S1[u & 0xFF]);
    context->r[1] = make_uint32(S0[v >> 24], S1[(v >> 16) & 0xFF],
                     S0[(v >> 8) & 0xFF], S1[v & 0xFF]);
    if (test==0)
    {
    	printf("zuc_1_13_14_C-%d:%x\n",i+1,W );
        printf("zuc_1_19_21_C-%d:%x\n",i+1,W1 );
        printf("zuc_1_20_22_C-%d:%x\n",i+1,W2 );
        printf("zuc_1_23_24_C-%d:%x\n",i+1,u );
        printf("zuc_1_23_25_C-%d:%x\n",i+1,v );
        printf("zuc_1_26_17_C-%d:%x\n",i+1,context->r[0] );
        printf("zuc_1_27_18_C-%d:%x\n",i+1,context->r[1] );
    }
    else
    {
    	if(i!=0)
    	{
    		printf("zuc_1_13_14_W-%d:%x\n",i,W );
    	}
        printf("zuc_1_19_21_W-%d:%x\n",i,W1 );
        printf("zuc_1_20_22_W-%d:%x\n",i,W2 );
        printf("zuc_1_23_24_W-%d:%x\n",i,u );
        printf("zuc_1_23_25_W-%d:%x\n",i,v );
        printf("zuc_1_26_17_W-%d:%x\n",i,context->r[0] );
        printf("zuc_1_27_18_W-%d:%x\n",i,context->r[1] );
    }
    return W;
}

// 比特重组·「
static inline void bit_reorganization(pzuc_context context)
{
    context->x[0] = ((context->lfsr[15] & 0x7FFF8000)<<1 ) | (context->lfsr[14] & 0xFFFF);
    context->x[1] = ((context->lfsr[11] & 0xFFFF) << 16) | (context->lfsr[9] >> 15);
    context->x[2] = ((context->lfsr[7] & 0xFFFF) << 16) | (context->lfsr[5] >> 15);
    context->x[3] = ((context->lfsr[2] & 0xFFFF) << 16) | (context->lfsr[0] >> 15);
}

// 导出功能

// 初始化阶段.
void zuc_init(pzuc_context context, const uint8_t* key, const uint8_t* iv)
{
     uint64_t i;
     int j;
    //展开密钥.
    for (i = 0; i < 16; ++i) {
        context->lfsr[i] = make_uint31(key[i], D[i],iv[i]);
        printf("zuc_1_30_7(%d)_0:%x\n",i,context->lfsr[i]);
    }
    memset(context->r, 0, 2 * sizeof(uint32_t));
    printf("zuc_1_30_17_0:%x\n",context->r[0]);
    printf("zuc_1_30_18_0:%x\n",context->r[1]);
    for (i = 0; i < 32; ++i) {
        bit_reorganization(context);//比特重组
        printf("zuc_1_31_8_C-%d:%x\n",i+1,context->x[0]);
        printf("zuc_1_31_9_C-%d:%x\n",i+1,context->x[1]);
        printf("zuc_1_31_10_C-%d:%x\n",i+1,context->x[2]);
        printf("zuc_1_31_11_C-%d:%x\n",i+1,context->x[3]);
        const uint32_t w = f(context,i,0);//
        lfsr_init(context->lfsr, w >> 1);//
        for (j = 0; j <16; ++j)
        {
        	printf("zuc_1_32_7(%d)_C-%d:%x\n",j,i+1,context->lfsr[j]);
        }
    }
}
//工作阶段
void zuc_generate_keystream(pzuc_context context, uint32_t keystream_buffer[], const size_t keystream_length) {
    uint64_t i;
    bit_reorganization(context);//第一轮
    printf("zuc_1_31_8_W-0:%x\n",context->x[0]);
    printf("zuc_1_31_9_W-0:%x\n",context->x[1]);
    printf("zuc_1_31_10_W-0:%x\n",context->x[2]);
    printf("zuc_1_31_11_W-0:%x\n",context->x[3]);
    f(context,0,1); // 丢弃F.的输出
    lfsr_shift(context->lfsr);
    for (i = 0; i <16; ++i)
    {
        printf("zuc_1_37_7(%d)_W-0:%x\n",i,context->lfsr[i]);
    }
    for (i = 0; i < keystream_length; ++i) {//密钥输出
        bit_reorganization(context);
        printf("zuc_1_31_8_W-%d:%x\n",i+1,context->x[0]);
        printf("zuc_1_31_9_W-%d:%x\n",i+1,context->x[1]);
        printf("zuc_1_31_10_W-%d:%x\n",i+1,context->x[2]);
        printf("zuc_1_31_11_W-%d:%x\n",i+1,context->x[3]);
        keystream_buffer[i] = f(context,i+1,1) ^ context->x[3];
        printf("zuc_1_15_16_W-%d:%x\n",i+1,keystream_buffer[i]);
        lfsr_shift(context->lfsr);
    }
}

