   ...
   //预处理：
    ...
    //为这个块初始化散列值：
        ...
        for(i = 0; i<64; i++)
        {
            ...
            printf("面板2:9(i=%d) %x\n",i,w[g]);
            printf("面板2:10(i=%d) %x\n",i,a+f+k[i]+w[g]);
            printf("面板2:11(i=%d) %x\n",i,LEFTROTATE ((a + f + k[i] + w[g]), r[i]));
            printf("面板2:12(i=%d) %x\n",i,b+LEFTROTATE ((a + f + k[i] + w[g]), r[i]));
            temp = d;
            d = c;
            c = b;
...
int main(int argc, char **argv)
{
    ...
    printf("面板1:13 %2.2x%2.2x%2.2x%2.2x\n",result[0],result[1],result[2],result[3]);
    printf("面板1:14 %2.2x%2.2x%2.2x%2.2x\n",result[4],result[5],result[6],result[7]);
    printf("面板1:15 %2.2x%2.2x%2.2x%2.2x\n",result[8],result[9],result[10],result[11]);
    printf("面板1:16 %2.2x%2.2x%2.2x%2.2x\n",result[12],result[13],result[14],result[15]);
    puts("");
    return 0;
}
