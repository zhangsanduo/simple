...
int set_pra(unsigned char _p[],unsigned char _a[],unsigned char _b[],unsigned char _n[]
	,unsigned char _Gx[],unsigned char _Gy[])
{
	FILE *fp;
	if((fp=fopen("var.text","w"))==NULL)
	{
		printf("Open Falied!");
		return -1;
	};
...
int Init_pra()
{
	...
	while (!feof(fp))									
	{
		...
		{
			case 1:
			fuchextods(strLine,SM2_p,SM2_NUMWORD*2);
			break;

			case 2:
			fuchextods(strLine,SM2_a,SM2_NUMWORD*2);
			break;

			case 3:
			fuchextods(strLine,SM2_b,SM2_NUMWORD*2);
			break;

			case 4:
			fuchextods(strLine,SM2_n,SM2_NUMWORD*2);
			break;

			case 5:
			fuchextods(strLine,SM2_Gx,SM2_NUMWORD*2);
			break;

			case 6:
			fuchextods(strLine,SM2_Gy,SM2_NUMWORD*2);
			break;
		...
		char d[],unsigned char R[],unsigned char S[])
{
	...
	int i;
	unsigned char rand[64],out[64],hex_rand[32],x1y1[64];
	...
	printf("ecc_2_9_8_0:");
	for(int i=0;i<len+SM2_NUMWORD;i++)
	{
		printf("%02x",M[i]);
	}
	printf("\n");
	...
	{
	   	rand32h(rand); //产生随机数
	   	print_hex(rand,64,out);
	   	fuchextods(out,hex_rand,32);
//	   	unsigned char test_rand[32]={0x6c,0xb2,0x8d,0x99,0x38,0x5c,0x17,0x5c,0x94,0xf9,0x4e,0x93,0x48,0x17,0x66,0x3f,0xc1,0x76,0xd9,0x25,0xdd,0x72,0xb7,0x27,0x26,0x0d,0xba,0xae,0x1f,0xb2,0xf9,0x6f};
//		memcpy(hex_rand,test_rand,32);
		printf("ecc_2_13_12_%d:",index);
		for(int i=0;i<SM2_NUMWORD;i++)
		{
			printf("%02x",hex_rand[i]);
		}
		printf("\n");

		//step3:generate k
		bytes_to_big(SM3_len/8,hex_rand,str);
		power(str,1,para_n,k);
		//step4:calculate kG
		ecurve_mult(k,G,KG);
		//step5:calculate r
		epoint_get(KG,KGx,KGy);

		big_to_bytes(SM2_NUMWORD,KGx,x1y1,TRUE);
		big_to_bytes(SM2_NUMWORD,KGy,x1y1+SM2_NUMWORD,TRUE);

		printf("ecc_2_15_14_%d:",index);
		...
		add(e,KGx,r);
		power(r,1,para_n,rem);
//		divide(r,para_n,rem);

		big_to_bytes(SM2_NUMWORD,rem,R,TRUE);
		printf("ecc_2_17_16_%d:",index);
		for(int i=0;i<SM2_NUMWORD;i++)
		{
			printf("%02x",R[i]);
		}
		...
		//judge r=0 or n+k=n?
		add(rem,k,rk);
		if( Test_Zero(rem) | Test_n(rk))
		{
			printf("ecc_2_17_18_%d:0\n",index);
			index++;
			continue;
		}
		printf("ecc_2_17_18_%d:-1\n",index);

		//step6:generate s
		incr(dA,1,z1);
		xgcd(z1,para_n,z1,z1,z1);
		multiply(rem,dA,z2);
		divide(z2,para_n,str);
		subtract(k,z2,z2);
		add(z2,para_n,z2);
		multiply(z1,z2,s);
		power(s,1,para_n,sem);

		big_to_bytes(SM2_NUMWORD,sem,S,TRUE);
		printf("ecc_2_20_19_%d:",index);
		for(int i=0;i<SM2_NUMWORD;i++)
		{
			printf("%02x",S[i]);
		}
		...
		if(Test_Zero(sem))
		{
			printf("ecc_2_20_21_%d:0\n",index);
			index++;
			continue;
		}
		printf("ecc_2_20_21_%d:-1\n",index);
		break;
	}
	big_to_bytes(SM2_NUMWORD,rem,R,TRUE);
	big_to_bytes(SM2_NUMWORD,sem,S,TRUE);
...
int SM2_Verify(unsigned char *message,int len,unsigned char ZA[],unsigned char Px[],unsigned
	...
	if (Test_Range(r))
	{	
		printf("ecc_3_11_10_0:-1\n");
		return ERR_OUTRANGE_R;
	}
	printf("ecc_3_11_10_0:0\n");
	//step2: test if s belong to [1,n-1]
	if (Test_Range(s))
	{
		printf("ecc_3_13_12_0:-1\n");
		return ERR_OUTRANGE_S;
	}
	printf("ecc_3_13_12_0:0\n");
	//step3,generate M
	M=(char *)malloc(sizeof(char)*(M_len+1));
	memcpy(M,ZA,SM3_len/8);
	memcpy(M+SM3_len/8,message,len);

	printf("ecc_3_15_14_0:");
	for(int i=0;i<len+SM2_NUMWORD;i++)
	{
		printf("%02x",M[i]);
	}
...
//	divide(t,para_n,rem);
	...
	big_to_bytes(SM2_NUMWORD,t,R1,TRUE);
	printf("ecc_3_19_18_0:");
	for(int i=0;i<SM2_NUMWORD;i++)
	{
		printf("%02x",R1[i]);
	}
	...
	if( Test_Zero(rem))
	{
		printf("ecc_3_19_20_0:0\n");
		return ERR_GENERATE_T;
	}
	printf("ecc_3_19_20_0:-1\n");
...
//	divide(RR,para_n,rem);
	...
	big_to_bytes(SM2_NUMWORD,rem,R1,TRUE);
	printf("ecc_3_24_23_0:");
	for(int i=0;i<SM2_NUMWORD;i++)
	{
		printf("%02x",R1[i]);
	}
	...
	{
		printf("ecc_3_24_25_0:0\n");
		return 0;
	}
	else
	{
		printf("ecc_3_24_25_0:-1\n");
...
int Sign(unsigned char in_message[],unsigned char in_prikey[])
{
	...
	if(temp)
		return temp;
	printf("ecc_2_20_22_0:");
    for(int i=0;i<SM2_NUMWORD;i++)
    {
        printf("%02x",r[i]);
    }
    printf("\n");
	printf("ecc_2_20_23_0:");
    for(int i=0;i<SM2_NUMWORD;i++)
    {
        printf("%02x",s[i]);
    }
