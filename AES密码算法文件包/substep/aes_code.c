 ...
 * 轮密钥加
 */
static void addRoundKey(int array[4][4], int round,int test) {
    if(test==0) 
   {
    if (round == 0) {
...
void aes(char *p, int plen, char *key){
    ...
    for(k = 0; k < plen; k += 16) { 
        ...
        addRoundKey(pArray, 0,0);//一开始的轮密钥加
        ...
        for(i = 1; i < 10; i++){//前9轮
            ...
            addRoundKey(pArray, i,0);
        }

        //第10轮
        subBytes(pArray, 10);//字节代换
        shiftRows(pArray, 10);//行移位
        addRoundKey(pArray, 10,0);
        printf("aes_1_13_17_0:");
...
void deAes(char *c, int clen, char *key) {
    ...
    for(k = 0; k < clen; k += 16) {
        ...
        addRoundKey(cArray, 10,1);

        int j=1;
        for(i = 9; i >= 1; i--,j++) {
            char x = 10-i;
            if(i==9)printf("aes_2_3_4_1:");
            else printf("aes_2_8_9_%d:",j);
            deSubBytes(cArray);
             if(i==9)printf("aes_2_3_5_1:");
            else printf("aes_2_8_10_%d:",j);
            deShiftRows(cArray);
            addRoundKey(cArray, i,1);
            if(i==9)printf("aes_2_3_4_1:");
            else printf("aes_2_8_12_%d:",j);
            deMixColumns(cArray);
        }
        printf("aes_2_13_14_10:");
        deSubBytes(cArray);
        printf("aes_2_13_15_10:");
        deShiftRows(cArray);

        addRoundKey(cArray, 0,1);
        printf("aes_2_13_17_0:");
